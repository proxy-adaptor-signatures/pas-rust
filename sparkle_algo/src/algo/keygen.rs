...
use crate::exn;
use crate::ThresholdCurve;

pub fn algo_keygen<C: ThresholdCurve>(
    server: &str,
    tr_uuid: &str,
    tn_config: &[u16; 2],
    context: &str,
    ...
    let mut rng = OsRng;
    let party_key = PartyKey::<C>::new(party_num_int, &mut rng);
    ...
    let mut dkg_com_vec: Vec<KeyGenProposedCommitment<C>> = round1_ans_vec
        .iter()
        .map(|m| json_to_obj(m))
        .collect::<Result<Vec<KeyGenProposedCommitment<C>>, _>>()?;
    ...
    let (invalid_peer_ids, valid_com_vec): (Vec<u16>, Vec<KeyGenCommitment<C>>) =
    ...
    let mut enc_keys: Vec<C::Point> = Vec::new();
    for i in 1..=parties {
        if i != party_num_int {
            enc_keys.push(
    ...
    for (k, i) in (1..=parties).enumerate() {
        if i != party_num_int {
            ...
            let key_i = &C::point_compress(&enc_keys[j]);
            let plaintext = C::scalar_to_bytes(&shares[k].get_value());
    ...
    let mut j = 0;
    let mut party_shares: Vec<Share<C>> = Vec::new();
    for i in 1..=parties {
        if i == party_num_int {
            ...
            let out_fe = Share::new_from(i, party_num_int, C::scalar_from_bytes_reduced(out_arr));
            party_shares.push(out_fe);
            j += 1;
        }
    }

    let signing_key: SigningKey<C> = match PartyKey::keygen_verify_share_construct_signingkey(
        party_shares.clone(),
        valid_com_vec.clone(),
        party_num_int,
    ) {
